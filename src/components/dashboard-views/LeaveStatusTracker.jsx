  ...
  return (
    <div className="flex flex-col pl-4">
      {approvalChain.map((level, i) => {
        ...
        const actionLogs = leave.auditLogs?.filter(log => nodeIdsInLevel.includes(log.nodeId) && ['APPROVED', 'REJECTED', 'PROPOSED_DATES'].includes(log.action)) || [];

        return (
          <div key={i} className={cn("relative flex items-start gap-4", i < approvalChain.length - 1 && "pb-6")}>
            {i < approvalChain.length - 1 && (
              <div className="absolute left-[5px] top-[18px] -bottom-[6px] w-0.5 bg-border" />
            )}
            <div className={cn(
              "relative z-10 w-3 h-3 mt-1.5 rounded-full ring-4 ring-background shrink-0",
                ...
                <div className="mt-2 space-y-1">
                  {actionLogs.map((log, idx) => (
                    <div key={idx} className="text-xs text-muted-foreground">
                      ...
                      {' '}{log.action === 'APPROVED' ? 'approved' : log.action === 'REJECTED' ? 'rejected' : 'proposed dates'}{' on '}
